...
BOOL XM_Init(void)
{
	if(!(mh=(XMHEADER *)_mikm_malloc(sizeof(XMHEADER)))) return 0;
	return 1;
}
...
static BOOL LoadPatterns(BOOL dummypat)
{
	...
	for(t=0;t<mh->numpat;t++) {
		...
		if(ph.numrows) {
			if(!(xmpat=(XMNOTE*)_mikm_calloc(ph.numrows*of.numchn,sizeof(XMNOTE))))
				return 0;
	...
	if(dummypat) {
		of.pattrows[t]=64;
		if(!(xmpat=(XMNOTE*)_mikm_calloc(64*of.numchn,sizeof(XMNOTE)))) return 0;
		for(v=0;v<of.numchn;v++)
