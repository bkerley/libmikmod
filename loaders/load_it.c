...
BOOL IT_Init(void)
{
	if(!(mh=(ITHEADER*)_mikm_malloc(sizeof(ITHEADER)))) return 0;
	if(!(poslookup=(UBYTE*)_mikm_malloc(256*sizeof(UBYTE)))) return 0;
	if(!(itpat=(ITNOTE*)_mikm_malloc(200*64*sizeof(ITNOTE)))) return 0;
	if(!(mask=(UBYTE*)_mikm_malloc(64*sizeof(UBYTE)))) return 0;
	if(!(last=(ITNOTE*)_mikm_malloc(64*sizeof(ITNOTE)))) return 0;
...
BOOL IT_Load(BOOL curious)
{
	...
	if(!(origpositions=_mikm_calloc(mh->ordnum,sizeof(UWORD)))) return 0;
	...
	if(!(paraptr=(ULONG*)_mikm_malloc((mh->insnum+mh->smpnum+of.numpat)*
