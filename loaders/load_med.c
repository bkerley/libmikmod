...
BOOL MED_Init(void)
{
	if (!(me = (MEDEXP *)_mikm_malloc(sizeof(MEDEXP))))
		return 0;
	if (!(mh = (MEDHEADER *)_mikm_malloc(sizeof(MEDHEADER))))
		return 0;
	if (!(ms = (MEDSONG *)_mikm_malloc(sizeof(MEDSONG))))
		return 0;
	return 1;
}
...
static BOOL LoadMEDPatterns(void)
{
	...
	if (!
		(mmd0pat =
		 (MMD0NOTE *)_mikm_calloc(of.numchn * (maxlines + 1),
...
static BOOL LoadMMD1Patterns(void)
{
	...
	if (!
		(mmd1pat =
		 (MMD1NOTE *)_mikm_calloc(of.numchn * (maxlines + 1),
...
BOOL MED_Load(BOOL curious)
{
	...
	if (!(ba = (ULONG *)_mikm_calloc(ms->numblocks, sizeof(ULONG))))
		return 0;
	...
	if ((mh->MEDEXPP) && (me->songname) && (me->songnamelen)) {
		...
		name = _mikm_malloc(me->songnamelen);
