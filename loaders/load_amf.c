...
BOOL AMF_Init(void)
{
	if(!(mh=(AMFHEADER*)_mikm_malloc(sizeof(AMFHEADER)))) return 0;
	if(!(track=(AMFNOTE*)_mikm_calloc(64,sizeof(AMFNOTE)))) return 0;
...
BOOL AMF_Load(BOOL curious)
{
	...
	/* read track table */
	if(!(track_remap=_mikm_calloc(mh->numtracks+1,sizeof(UWORD))))
		return 0;
