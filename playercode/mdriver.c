...
BOOL MikMod_SetNumVoices_internal(int music, int sfx)
{
	...
	if(md_sngchn+md_sfxchn)
		md_sample=(SAMPLE**)_mikm_calloc(md_sngchn+md_sfxchn,sizeof(SAMPLE*));
	if(md_sfxchn)
		sfxinfo = (UBYTE *)_mikm_calloc(md_sfxchn,sizeof(UBYTE));
	...
	if(cmdline) {
		...
		if((buf)&&((buf==cmdline)||(*(buf-1)==','))) {
			...
			if(*ptr=='=') {
				...
				ret=_mikm_malloc((1+ptr-buf)*sizeof(CHAR));
				if(ret)
