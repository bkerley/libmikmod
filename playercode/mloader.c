...
MIKMODAPI CHAR* MikMod_InfoLoader(void)
{
	...
	if(len)
		if((list=_mikm_malloc(len*sizeof(CHAR)))) {
			list[0]=0;
...
BOOL ReadComment(UWORD len)
{
	if(len) {
		...
		if(!(of.comment=(CHAR*)_mikm_malloc(len+1))) return 0;
...
BOOL ReadLinedComment(UWORD len,UWORD linelen)
{
	...
	if (len) {
		if(!(tempcomment=(CHAR*)_mikm_malloc(len+1))) return 0;
		if(!(storage=(CHAR*)_mikm_malloc(linelen+1))) {
			free(tempcomment);
			return 0;
		}
		...
		if(total>lines) {
			if(!(of.comment=(CHAR*)_mikm_malloc(total+1))) {
				free(storage);
				free(tempcomment);
				return 0;
			}
...
BOOL AllocPositions(int total)
{
	...
	if(!(of.positions=_mikm_calloc(total,sizeof(UWORD)))) return 0;
	return 1;
}
...
BOOL AllocPatterns(void)
{
	...
	if(!(of.patterns=(UWORD*)_mikm_calloc((ULONG)(of.numpat+1)*of.numchn,sizeof(UWORD)))) return 0;
	if(!(of.pattrows=(UWORD*)_mikm_calloc(of.numpat+1,sizeof(UWORD)))) return 0;
...
BOOL AllocTracks(void)
{
	...
	if(!(of.tracks=(UBYTE **)_mikm_calloc(of.numtrk,sizeof(UBYTE *)))) return 0;
	return 1;
}
...
BOOL AllocInstruments(void)
{
	...
	if(!(of.instruments=(INSTRUMENT*)_mikm_calloc(of.numins,sizeof(INSTRUMENT))))
		return 0;
...
BOOL AllocSamples(void)
{
	...
	if(!(of.samples=(SAMPLE*)_mikm_calloc(of.numsmp,sizeof(SAMPLE)))) return 0;
...
CHAR *DupStr(CHAR* s,UWORD len,BOOL strict)
{
	...
	if((d=(CHAR*)_mikm_malloc(sizeof(CHAR)*(len+1)))) {
...
static MODULE *ML_AllocUniMod(void)
{
	...
	return (mf=_mikm_malloc(sizeof(MODULE)));
}
